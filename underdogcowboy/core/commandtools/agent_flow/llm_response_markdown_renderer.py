...
class LLMResponseRenderer:
    ...
    def get_renderable(self, model_response: str, title: Optional[str] = None):
            """
            Cleans, formats, and returns a Rich renderable of the model response.

            :param model_response: The raw model response string.
            :param title: Optional title for the response.
            :return: A Rich renderable (Markdown instance)
            """
            self.logger.info("Processing and getting renderable of the model response.")
            cleaned = self.clean_response(model_response)
            formatted = self.format_markdown(cleaned)
            # Create Markdown-formatted text with an optional heading
            if title:
                ...
                markdown_text = f"### 💬 **LLM Response**\n\n{formatted}"
            renderable = Markdown(markdown_text)
            self.logger.info("Completed processing and getting renderable.")
            return renderable