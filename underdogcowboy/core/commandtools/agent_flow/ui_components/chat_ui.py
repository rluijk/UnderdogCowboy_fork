...
class ChatUI(Static):
    ...
    def replace_loading_indicator(self, response_text: str):
        """Replace the loading indicator with the LLM response."""
        # Remove the loading indicator ("...") from the chat history
        if self.loading_message_id:
            # Remove the last message which is the loading indicator
            self.chat_history_text = self.chat_history_text.rstrip("...\n#### Assistant:\n")
        # Append the assistant's response
        self.chat_history_text += f"\n#### Assistant:\n{response_text}"
