...
class AgentDialogManager(DialogManager):
    ...
    def __init__(self, agent_inputs: List[Union[Type[Agent], Agent]], model_name: Optional[str] = None, use_tracing: bool = False, **kwargs: Any) -> None:
        ...
        # Initialize a lock for managing access to active_agent
    ...
    async def message(self, agent: Agent, user_input: str) -> 'Response':
        # Process a user input message using the specified agent
        with self.tracer.trace(f"Agent Dialog: {agent.name}"):
            self.tracer.log("User Input", {"input": user_input})
            ...
            # Acquire the lock before setting the active agent
            async with self._active_agent_lock:
                self.active_agent = agent

            processor = self.processors[agent]

            # Process the user input message through the processor
            result = await processor.process_single_message(user_input)
    ...
    async def ask_agents(self, agent_names: List[str], question: str) -> List[str]:
        async def ask_single_agent(agent_name: str) -> str:
            agent = next((a for a in self.agents if a.name == agent_name), None)
            if agent is None:
                return f"Agent {agent_name} is not in the conversation."
            
            response = await self.message(agent, question)
            
            # Log the interaction through the meta processor
            self.meta_processor.process_single_message(f"User: Question to {agent_name}: {question}")
            self.meta_processor.process_single_message(f"{agent_name}: {response.content}")
            
            return f"{agent_name}: {response.content}"

        # Use asyncio.gather to run all agent queries concurrently
        responses = await asyncio.gather(*(ask_single_agent(name) for name in agent_names))
        return responses

    def _create_meta_processor(self) -> CommandProcessor:
        """Helper method to create a CommandProcessor for the meta timeline."""
        if not self.model_name:
            self.model_name = self.config_manager.select_model()
        
        model = ModelManager.initialize_model(self.model_name)
        return CommandProcessor(self.meta_timeline, model)

    def invite_agent(self, agent_name: str) -> str:
        # Check if the agent is already present
        if agent_name in self.agents:
            return f"Agent {agent_name} is already in the conversation."
        
        # Use the superclass method to initialize and prepare the agent
        new_agent = self._initialize_agent(Agent(agent_name))
        self.prepare_agent(new_agent)
        
        # Log the invitation message through the meta processor
        self.meta_processor.process_single_message(f"System: Agent {agent_name} has been invited to the conversation.")
        
        return f"Agent {agent_name} has been invited to the conversation."


    def get_full_conversation(self) -> str:
        # Use the meta processor to retrieve the entire conversation as a string
        messages = self.meta_processor.timeline.get_messages()
